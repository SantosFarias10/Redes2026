...
>[!tip] Se implementa sólo en los sistemas finales (no en routers intermedios)

Mejora los servicios de la capa de red
- Por ejemplo: Retransmite paquetes perdidos en redes no orientadas a la conexión.
- Por ejemplo: Regula la tasa de transmisión ante congestión en la red.
...
![](Screenshot_2026-06-22-11-14-15_24983.png)
...
### Modelo de servicio:
- Flujo de bytes confiable (stream): la aplicación ve una 'tubería' continúa de bytes entre origen y destino
...
- La aplicación envía un flujo continuó de bytes a TCP. TCP no conoce los límites de los mensajes de la aplicación: solo ve un **stream de bytes**.
...
1. Encabezado fijo — 20 bytes. Contiene puertos, números de secuencia/ACK, flags y control de flujo. (fijate que son las primeras 5 filas del cuadro $5 * 32$bits = 160 bits /8= 20 bytes)
...
#### Estructura del encabezado
![](Screenshot_2026-06-22-11-23-37_26677.png)
